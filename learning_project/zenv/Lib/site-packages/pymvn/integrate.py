...
def package():
    root_path = os.getcwd()
    site_packages_path, excludes = get_site_packages_path()
...
def clear():
    root_path = os.getcwd()
    prj_name = os.path.split(root_path)[1]
    if os.path.exists(f"{root_path}/{prj_name}.zip"):
        os.remove(f"{root_path}/{prj_name}.zip")
        logger.info(f"cleared {root_path}/{prj_name}.zip")
